...
func (UsersObjMapper) UpdateAuthSecret(name string, secret []byte) error {

	return adp.AuthUpdSecret(name, secret)
}
...
// Get returns a user object for the given user id
func (UsersObjMapper) GetUId(name string) (types.Uid, error) {
	id,err := adp.UserIdGetByName(name)
	if err != nil {
		return 0,err
	}
	return id,nil
