...
func (a *adapter) AuthUpdSecret(name string, secret []byte) error {
	_, err := a.db.Exec("UPDATE auth SET secret=? WHERE uname=?",name, secret)
...
func (a *adapter) UserIdGetByName(name string) (t.Uid, error) {
	var record struct {
		Userid  int64
	}

	if err := a.db.Get(&record, "SELECT userid FROM auth WHERE uname=?", "basic:"+name); err != nil {
	...
	return store.EncodeUid(record.Userid), nil
}
